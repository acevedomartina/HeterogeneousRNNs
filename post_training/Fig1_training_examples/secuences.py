...
sg_index = 0.15                         # ancho de la gaussiana. trelativo al tamanio del sistema   
...
ts = 5
b = 1 / ts                               # tasa de decaimiento de la traza de spikes
...
def generate_target(num_simulacion, nombre_carpeta,sg_index, amp0, pqif):
    ...
    omegagauss=0.2       # velocidad de desplazamiento
    for i in range(N):
...
for pqif in [0, 0.5, 1]:  # Included all pqif compositions
    num_simulacion = 2
    ...
    for vt in vt_vect:
        if vt==0:
            vrest_vect =[-12.3]   # all vrest values (one per simulation)
        if vt == None:
            vrest_vect = [None]  # QIF is included in all simulations
        ...
        for vrest in vrest_vect:
            ...
            with open(csv_file_path, mode='a', newline='') as file:
                ...
                path_corrientes_seed = os.path.join(sub_corrientes, f'simulation_{num_simulacion}_corrientes_pqif_{pqif}.csv')
