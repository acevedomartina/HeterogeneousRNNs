...
def crear_carpetas(num_simulacion, omegagauss=None, ts=None): 
    nombre_carpeta = f"simulacion_{num_simulacion}"
    if not os.path.exists(nombre_carpeta):
        os.makedirs(nombre_carpeta)
    
    # Si se especifica omegagauss y ts, crear subcarpeta específica
    if omegagauss is not None and ts is not None:
        carpeta_omega_ts = crear_subcarpeta(nombre_carpeta, f"omega_{omegagauss}")
        
        sub_pesos = crear_subcarpeta(carpeta_omega_ts, f"matrices_pesos")
        sub_corrientes = crear_subcarpeta(carpeta_omega_ts, f"corrientes")
        sub_inputs = crear_subcarpeta(carpeta_omega_ts, f"inputs")
        sub_outputs = crear_subcarpeta(carpeta_omega_ts, f"outputs")
        sub_nspikes = crear_subcarpeta(carpeta_omega_ts, f"nspikes")
        
        return nombre_carpeta, carpeta_omega_ts, sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes
    else:
...
def crear_archivo_parametros(filename_resultados, num_simulacion, nombre_carpeta, vt, vrest):
    data_parametros = {
        'N': [N],
        'p': [p],
        'gsyn': [gsyn],
...
def save_target(target, sg_index, omegagauss, amp0, num_simulacion, carpeta_omega_ts, pqif, ts):
    """
    Guarda el target y sus parámetros en la carpeta específica
    """
    target_df = pd.DataFrame(target.T, columns=[f'Neurona_{i}' for i in range(N)])
    nombre_archivo_target = f'targets_pqif_{pqif}.csv'
    csv_target_path = os.path.join(carpeta_omega_ts, nombre_archivo_target)
    target_df.to_csv(csv_target_path, index=False)

    data = {'sg_index': [sg_index], 'omegagauss': [omegagauss], 'amp0': [amp0], 'ts': [ts]}
    df = pd.DataFrame(data)
    nombre_archivo = f'targets_parametros_pqif_{pqif}.csv'
    csv_target_path = os.path.join(carpeta_omega_ts, nombre_archivo)
...
# ========== FUNCIÓN PARA CORRER UNA SEMILLA ==========
def run_single_seed(seed, omegagauss, ts, pqif, num_simulacion, vt, vrest, target,
                    sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes,
                    carpeta_omega_ts, nombre_carpeta):
    """
    Ejecuta la simulación para una semilla específica
    """
    ...
    path_Iext = os.path.join(carpeta_omega_ts, 
    ...
    # Archivo dw
    filename_dw = os.path.join(carpeta_omega_ts, 
                              f'dw_pqif_{pqif}_seed_{seed}.csv')
    
    # Lista para acumular resultados de esta semilla
    resultados_seed = []
        ...
        for iloop in range(nloop):
            ...
            for it in range(itmax):
                ...
                if iloop in [nloop_train + 1, nloop - 1] and it % 20 == 0:
                    corrientes_buffer.append([omegagauss, ts, pqif, seed, iloop, it, 
                                            II[0], v[0], II[1], v[1], 
            ...
            # Acumular resultados
            resultados_seed.append([omegagauss, ts, b, pqif, seed, iloop, sigma2, tau_rec, 
                                  tau_div, tau_con, tau_chn])
    
    print(f"      ✓ Seed {seed} completado")
    
    return resultados_seed


# ========== FUNCIÓN PRINCIPAL DE SIMULACIÓN ==========
def run_simulation_omega_ts(omegagauss, ts, pqif, num_simulacion, vt, vrest, sg_index, amp0,
                            N, N2, p, gsyn, nloop, nloop_train, cant_seed,
                            dt, itmax, itstim, amp_corriente, alpha, sigman, iout):
    """
    Ejecuta la simulación para un valor específico de omegagauss, ts y pqif
    Paraleliza sobre las semillas
    """
    
    # Calcular b a partir de ts
    b = 1 / ts
    
    print(f"\n{'='*60}")
    print(f"Simulación {num_simulacion} - Procesando omegagauss={omegagauss}, ts={ts}, pqif={pqif}")
    print(f"vt={vt}, vrest={vrest}, b={b}")
    print(f"Paralelizando {cant_seed} semillas...")
    print(f"{'='*60}\n")
    ...
    # Crear carpetas específicas para este omegagauss y ts
    nombre_carpeta, carpeta_omega_ts, sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes = crear_carpetas(num_simulacion, omegagauss, ts)
    ...
    save_target(target, sg_index=sg_index, omegagauss=omegagauss, 
               ...
               carpeta_omega_ts=carpeta_omega_ts, pqif=pqif, ts=ts)
    
    # Preparar archivo de corrientes (crear con header)
    path_corrientes_seed = os.path.join(sub_corrientes, 
                                       f'corrientes_pqif_{pqif}.csv')
    
    with open(path_corrientes_seed, mode='w', newline='') as file_:
        writer_ = csv.writer(file_)
        writer_.writerow(['omegagauss', 'ts', 'pqif', 'seed', 'iloop', 'it', 'II_0', 'v_0', 
                        'II_1', 'v_1', 'II_N2+1', 'v_N2+1', 'II_N2+2', 'v_N2+2'])
    
    # PARALELIZACIÓN SOBRE SEMILLAS
    all_results = Parallel(n_jobs=-1, verbose=5)(
        delayed(run_single_seed)(
            seed, omegagauss, ts, pqif, num_simulacion, vt, vrest, target,
            sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes,
            carpeta_omega_ts, nombre_carpeta
        )
    ...
    # Consolidar resultados de todas las semillas
    ...
    with open(csv_file_path, 'a', newline='') as file_res:
        ...
        for resultados_seed in all_results:
            writer_res.writerows(resultados_seed)
    
    print(f"✓ omegagauss={omegagauss}, ts={ts}, pqif={pqif} completado para simulación {num_simulacion}\n")
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("SIMULACIONES CON DOBLE PARALELIZACIÓN:")
    print("  1) Nivel externo: combinaciones ts x pqif")
    print("  2) Nivel interno: semillas")
    print("="*70)
    
    # Valor fijo de omegagauss
    omegagauss = 0.5
    
    # Lista de valores ts a simular
    ts_values = [5]
    ...
    print(f"Valor fijo de omegagauss: {omegagauss}")
    print(f"Valores de ts por simulación: {len(ts_values)}")
    print(f"Valores de pqif por simulación: {len(pqif_values)}")
    print(f"Semillas por combinación: {cant_seed}")
    print(f"Total de combinaciones por simulación: {len(ts_values) * len(pqif_values)}")
    print(f"Total de ejecuciones paralelas por simulación: {len(ts_values) * len(pqif_values) * cant_seed}\n")
    ...
    for num_simulacion, config in enumerate(configs, start=1):
        ...
        crear_archivo_parametros(filename_resultados, num_simulacion, 
                                nombre_carpeta, vt, vrest=vrest)
        
        # Crear archivo de resultados con header (incluyendo ts y b)
        csv_file_path = os.path.join(nombre_carpeta, filename_resultados)
        with open(csv_file_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['omegagauss', 'ts', 'b', 'pqif', 'seed', 'nloop', 'sigma2', 'tau_rec',
                           'tau_div', 'tau_con', 'tau_chn'])
        
        # Crear todas las combinaciones de (ts, pqif)
        combinaciones = [(ts, pqif) for ts in ts_values for pqif in pqif_values]
        
        # Paralelizar sobre todas las combinaciones (nivel externo)
        # Cada combinación internamente paraleliza las semillas
        results = Parallel(n_jobs=-1, verbose=10)(
            delayed(run_simulation_omega_ts)(
                omegagauss, ts, pqif, num_simulacion, vt, vrest, sg_index, amp0,
                N, N2, p, gsyn, nloop, nloop_train, cant_seed,
                dt, itmax, itstim, amp_corriente, alpha, sigman, iout
            )
            for ts, pqif in combinaciones
        )
