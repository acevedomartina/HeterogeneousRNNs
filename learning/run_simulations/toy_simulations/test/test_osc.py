...
cant_seed = 2          # Independent simulations
ts = 5                  # 
...
def crear_carpetas(num_simulacion): 
    ...
    sub_targets = crear_subcarpeta(nombre_carpeta, f"simulation_{num_simulacion}_targets")

    return nombre_carpeta, sub_act, sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes, sub_targets
...
def save_target(target, phase, omega, romega_vec, amp, amp0, num_simulacion, nombre_carpeta, pqif, seed=None, sub_targets=None):

    # Choose the destination folder
    if sub_targets is None:
        folder = nombre_carpeta                # legacy behaviour
    else:
        folder = sub_targets                    # new dedicated folder
    ...
    if seed is None:
        fname_par = f'simulation_{num_simulacion}_targets_parameters_pqif_{pqif}.csv'
    else:
        # Save target per seed
        fname_par = (f'simulation_{num_simulacion}_targets_parameters_pqif_{pqif}_seed_{seed}.csv')

    df.to_csv(os.path.join(folder, fname_par), index=False)

    # Target matrix

    target_df = pd.DataFrame(target.T, columns=[f'Neurona_{i}' for i in range(N)])
    if seed is None:
        fname_tar = f'simulation_{num_simulacion}_targets_pqif_{pqif}.csv'
    else:
        fname_tar = f'simulation_{num_simulacion}_targets_pqif{pqif}_seed_{seed}.csv'

    target_df.to_csv(os.path.join(folder, fname_tar), index=False)
...
def run_single_seed(seed, pqif, num_simulacion, vt, vrest,
                    ...
                    sub_inputs, sub_outputs, sub_nspikes, sub_targets):
    """
    ...
    # Generate target per seed
    target, amp, phase, omega, romega_vec, amp0_local = generate_target(
        romega1=1, romega2=5, amp0=amp0)

    save_target(target, phase, omega, romega_vec,
                amp, amp0_local,
                num_simulacion, nombre_carpeta, pqif, seed=seed, sub_targets=sub_targets)
...
def run_pqif_simulation(pqif, num_simulacion, vt, vrest,
    ...
    for seed in range(cant_seed):  # run each seed sequentially instead
        seed_results, corrientes_buffer = run_single_seed(
            seed, pqif, num_simulacion, vt, vrest,
            N, N2, p, gsyn, nloop, nloop_train,
            dt, itmax, itstim, amp_corriente, alpha, sigman,
            b, iout, nombre_carpeta, sub_pesos, sub_corrientes,
            sub_inputs, sub_outputs, sub_nspikes, sub_targets)
...
if __name__ == '__main__':
    ...
    for num_simulacion, config in enumerate(configs, start=1):
        ...
        nombre_carpeta, sub_act, sub_pesos, sub_corrientes, sub_inputs, sub_outputs, sub_nspikes, sub_targets = crear_carpetas(num_simulacion)
