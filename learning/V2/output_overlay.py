...
for dyn in dynamics:  # If not both dynamics, change above in parameters
    ...
    nrows=1,
    ncols=len(simulation_number),
    figsize=(20, 8),          
    constrained_layout=True   
    )

    for idx_pqif, pqif in enumerate(pqif_homogenous):


        for idx_sim, (idx, f) in enumerate(zip(simulation_number, slope)):
            ...
            ax.plot(np.arange(N_neurons), lif_mean, color=col)
            ax.plot(np.arange(N_neurons), qif_mean, color='steelblue')

            ax.set_box_aspect(1)

            ax.set_ylabel("Presynaptic activity (mean over time)")
